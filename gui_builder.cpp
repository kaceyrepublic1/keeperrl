...
SGuiElem GuiBuilder::drawKeeperHelp(const GameInfo& info) {
  ...
  auto handleHelpElement = [&](const ScriptedHelpInfo& elem) {
    if (elem.scriptedId.empty()) {
      if (elem.title) {
          lines.addElem(WL(label, *elem.title, Color::YELLOW));
          lines.addSpace(5);
      }
    } else {
        ScriptedHelpInfo buttonInfo = elem;
        if (!buttonInfo.viewId) {
            buttonInfo.viewId = ViewId("book");
        }
        if (!buttonInfo.title) {
            buttonInfo.title = TString(buttonInfo.scriptedId);
        }
        addScriptedButton(buttonInfo);
    }
  };
